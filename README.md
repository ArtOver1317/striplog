...
### ❌ Arguments are still evaluated (unless you use the lazy overload)

```csharp
// BAD: JsonUtility.ToJson() runs before Log.Debug is called
Log.Debug("State", $"Data: {JsonUtility.ToJson(bigObject)}");
...
### ❌ `#if` inside a method does not strip the call

```csharp
// BAD: the call to MyLog("...") still exists in IL
...
// GOOD: [Conditional] removes the call site
[Conditional("UNITY_EDITOR")]
public static void MyLog(string msg) => Debug.Log(msg);
```
...
```
https://github.com/ArtOver1317/striplog.git#v0.2.0
```
